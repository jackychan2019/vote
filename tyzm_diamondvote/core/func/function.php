...
function is_weixin(){
	...
	if(strpos($useragent, 'MicroMessenger') === false && strpos($useragent, 'Windows Phone') === false  && strpos($useragent, 'QQ') === false ){
...
function  ip2address($ip){
	...
	//$params 待签名数组
//$app_key 渠道秘钥	
function make_sign($params,$app_key)
{
    $str = '';
    ksort($params);
...
function check_sign($params,$app_key){
    ...
    $mysign = md5($str);
    if($mysign!=$sign){
		file_put_contents(IA_ROOT. '/addons/tyzm_diamondvote/log'.date("Y-m-d").'.txt', date("h:i:sa").'--->签名不匹配:'.json_encode($params).PHP_EOL, FILE_APPEND);
		return false;
	}else return true;
}
