...
if ($_W['ispost']) {
		...
		if(!empty($configdata['ipplace'])){
			$iptotal = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->tableredpack) . " WHERE uniacid = " . $_W['uniacid'] . " AND rid=" . $rid . "  and type = 0  AND CONCAT(`user_ip`) LIKE '%{$_W['clientip']}%'");
		...
		$everyonetotal = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tableredpack) . " WHERE rid = :rid  AND openid = :openid and type = :type ", array(
            ':rid' => $rid,':openid'=> $oauth_openid,":type" => 0
        ));
